...
from app.services.news_event_extractor import news_event_extractor, _simple_severity_fallback
...
Focus on trading relevance."""
    ...
    def score_headline_severities(
        ...
        Delegates to production news_event_extractor (single source of truth).
        Falls back to cheap keyword fallback.
        """
        ...
        try:
            ...
            logger.warning(f"[news_analyzer] extractor delegation failed for {symbol}: {e}")
            ...
            if not clean:
                return []
            return [
                {**_simple_severity_fallback(h), "headline": h}
                for h in clean
            ]
